...
matrix[0, :] = np.zeros(len(seq2)+1)
matrix[:, 0] = np.zeros(len(seq1)+1)
...
for row_index in range(1, len(seq1)+1):
    ...
    for col_index in range(1, len(seq2)+1):
        ...
        if seq1[row_index-1] == seq2[col_index-1]:
            # match
            next_value = np.amax([0, costs[0]+matrix[row_index-1, col_index-1],
                                 matrix[row_index-1, col_index]+costs[2],
                                 matrix[row_index, col_index-1]+costs[2]])
        else:
            # missmatch
            next_value = np.amax([0, costs[1] + matrix[row_index - 1, col_index - 1],
                                 matrix[row_index - 1, col_index] + costs[2],
                                 matrix[row_index, col_index - 1] + costs[2]])
        matrix[row_index, col_index] = next_value

print("Local alignment score: "+str(matrix[-1,-1]))
print(matrix)
...
def tree(row, col, align1, align2):
    ...
    if (row == 0 and col == 0) or (score_current == 0):
...
# Il punto di partenza è la posizione con il valore più alto nella matrice!
# per ciascun punto max:
for coordinate in np.argwhere(matrix.max() == matrix):
    tree(coordinate[0], coordinate[1], align1, align2)
